...
describe("download-proxy fetch", () => {
	it("GET on a present .kernel → 200 + body + one kernel_download event", async () => {
		...
		// The queue carries the tagged-union envelope the consumer routes on —
		// `{ kind: 'event', event }`, NOT a bare event (which `splitBatch` acks
		// but never inserts, silently dropping the row).
		const msg = sent[0] as { kind: string; event: Record<string, unknown> };
	...
	it("GET with X-Substrate-Anonymous-Id header → event carries that anonymous_id", async () => {
		...
		expect(sent).toHaveLength(1);
		expect(
			(sent[0] as { event: Record<string, unknown> }).event.anonymous_id,
		).toBe(anon);
	});
	...
	it("rejects a malformed anonymous_id and falls back to a fresh UUID", async () => {
		...
		const req = makeRequest("GET", `/${FIXTURE_KEY}`, {
			"X-Substrate-Anonymous-Id": bad,
		});
		const ctx = createExecutionContext();
		await worker.fetch(req, testEnv, ctx);
		await waitOnExecutionContext(ctx);

		const id = (sent[0] as { event: Record<string, unknown> }).event
	...
	it("GET with Range → 206 + Content-Range + NO event (resumable downloads do not over-count)", async () => {
		...
		expect(res.headers.get("Content-Range")).toBe(
			`bytes 0-15/${FIXTURE_BODY.length}`,
		);
	...
	it("GET SHA256SUMS → text/plain + 200 + one event with linux-SHA256SUMS package", async () => {
		...
		expect(sent).toHaveLength(1);
		const event = (sent[0] as { event: Record<string, unknown> }).event;
		expect((event.properties as Record<string, unknown>).package).toBe(
			"linux-SHA256SUMS",
		);
	...
	describe("GET / — kernels listing page", () => {
		...
		beforeEach(async () => {
			...
			await env.KERNELS.put(
				SUMS_KEY,
				new TextEncoder().encode("abcd  " + X86_KEY + "\n"),
			);
		});
		...
		it("GET / → 200 HTML listing the artifacts, no analytics emit", async () => {
			...
			expect(body).toMatch(
				/<nav class="top">\s*<a [^>]*github\.com\/agx-runtime[^"]*"[^>]*>GitHub<\/a>\s*<\/nav>/,
			);
		...
		it("GET / with a write key configured → injects the reverse-proxied SDK loader", async () => {
			...
			expect(body).toContain(
				'rudderanalytics.load("test-web-key",origin+"/_data",',
			);
	...
	describe("GET /_data/* — RudderStack SDK reverse proxy", () => {
		...
		it("GET /_data/sourceConfig/?writeKey=<known> → 200 + valid source config", async () => {
			const req = makeRequest(
				"GET",
				`/_data/sourceConfig/?writeKey=${WRITE_KEY}`,
			);
			const ctx = createExecutionContext();
		...
		it("POST /_data/v1/page → rewrites to <dataPlane>/v1/batch upstream", async () => {
			...
			try {
				...
				expect(fetchSpy).toHaveBeenCalledTimes(1);
				const [upstreamUrl, upstreamInit] = fetchSpy.mock.calls[0] as [
					string,
					RequestInit,
				];
				...
				expect(fwdHeaders.get("Authorization")).toBe(
					`Basic ${btoa(`${WRITE_KEY}:`)}`,
				);
